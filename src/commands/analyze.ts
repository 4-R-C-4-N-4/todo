...
import type { AnalysisType, AnalysisEntry } from '../types.js';

const VALID_ANALYSIS_TYPES: AnalysisType[] = ['blame', 'hypothesis', 'evidence', 'conclusion'];
...
export function registerAnalyze(program: Command): void {
  program
    ...
    .requiredOption('--type <type>', 'analysis type: blame|hypothesis|evidence|conclusion')
    .requiredOption('--content <text>', 'analysis content')
    .option('--confidence <level>', 'confidence: low|medium|high')
    .option('--supporting <indices>', 'comma-separated indices of supporting evidence, e.g. "0,1"')
    .action((id: string, opts) => {
      try {
        const analysisType = opts.type as string;
        if (!VALID_ANALYSIS_TYPES.includes(analysisType as AnalysisType)) {
          console.error(`Error: invalid type '${analysisType}'. Must be one of: ${VALID_ANALYSIS_TYPES.join(', ')}`);
          process.exit(1);
        }

        if (opts.confidence && !VALID_CONFIDENCE.includes(opts.confidence as string)) {
          console.error(`Error: invalid confidence '${opts.confidence}'. Must be one of: ${VALID_CONFIDENCE.join(', ')}`);
          process.exit(1);
        }
        ...
        console.log(`Added analysis entry [${ticket.analysis.length - 1}] to ${ticket.id}`);
      } catch (err) {
        handleError(err);
      }
    });
