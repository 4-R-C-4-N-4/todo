    ...
    .action((opts) => {
      try {
        ...
        if (strategy !== 'fingerprint' && strategy !== 'file-line') {
          console.error(`Error: invalid strategy '${strategy}'. Must be one of: fingerprint, file-line, semantic`);
          process.exit(1);
        }
        ...
        const pairs = findDuplicates(tickets, strategy as 'fingerprint' | 'file-line');
        ...
        if (!apply) {
          for (const p of pairs) {
            console.log(`Possible duplicate: ${p.ticket1} ↔ ${p.ticket2} (${p.reason})`);
          }
          console.log(`Found ${pairs.length} potential duplicate pair(s). Run with --apply to link them.`);
          return;
        }
        ...
        for (const p of pairs) {
          ...
          console.log(`Linked ${p.ticket1} ↔ ${p.ticket2} as related (${p.reason})`);
        }
