...
export function registerLink(program: Command): void {
  program
    ...
    .requiredOption('--to <target>', 'target: ticket ID prefix, git commit SHA, or file path')
    .option('--relation <type>', 'for ticket targets: depends_on|blocks|related|duplicates', 'related')
    ...
    .action((id: string, opts) => {
      try {
        ...
        if (!VALID_RELATIONS.includes(relation)) {
          console.error(`Error: invalid relation '${relation}'. Must be one of: ${VALID_RELATIONS.join(', ')}`);
          process.exit(1);
        }
        ...
        try {
          ...
          if (commitExists(target, repoRoot)) {
            ...
            console.error(`Error: Cannot resolve target '${target}' as ticket, commit, or file`);
            process.exit(1);
          }
        }
        ...
        if (resolvedType === 'ticket') {
          ...
          if (!ticket.relationships.linked_commits) ticket.relationships.linked_commits = [];
          ...
          const fileRef: { path: string; commit?: string; note?: string } = { path: target };
