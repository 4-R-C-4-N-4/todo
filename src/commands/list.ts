...
import type { State, Ticket, TicketType } from "../types.js";
...
function formatTable(tickets: Ticket[]): string {
	...
	const rows = tickets.map((t) => [
		...
		(t.state === "active" || t.state === "blocked") && t.work?.branch
			? t.work.branch
			: "",
	]);
...
export function registerList(program: Command): void {
	program
		...
		.option(
			"--sort <field>",
			"sort by: updated (default), created, type, state",
			"updated",
		)
		...
		.action((opts) => {
			...
			const dir =
				stateFilter && TERMINAL_STATES.includes(stateFilter) ? "done" : "open";
