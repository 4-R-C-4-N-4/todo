...
const VALID_SOURCES: SourceType[] = ['log', 'test', 'agent', 'human', 'comment'];
...
export function registerNew(program: Command): void {
  program
    ...
    .action((summaryArg: string | undefined, opts) => {
      ...
      if (opts.tags) {
        tags = (opts.tags as string).split(',').map(t => t.trim()).filter(t => t.length > 0);
      }
      ...
      // Handle parent
      if (opts.parent) {
        const { readTicketByPrefix } = require('../ticket.js') as typeof import('../ticket.js');
        try {
          const parentTicket = readTicketByPrefix(repoRoot, opts.parent as string);
          if (!parentTicket.relationships) parentTicket.relationships = {};
          if (!parentTicket.relationships.children) parentTicket.relationships.children = [];
