...
import { listTickets, writeTicket, generateId } from '../ticket.js';
  ...
  program
    ...
    .action((opts) => {
      try {
        ...
        const scanPatterns = config.intake?.scan_patterns ?? ['TODO', 'FIXME', 'HACK', 'XXX'];
        const scanExclude = config.intake?.scan_exclude ?? ['.todo', 'node_modules', '.git', 'dist', 'build'];
        ...
        for (const match of matches) {
          ...
          const id = generateId('comment', match.text + match.file + match.line, createdAt);
