...
function formatTicketDetail(ticket: Ticket): string {
  ...
  if ('test_file' in ticket.source && ticket.source.test_file) {
    const fn = 'test_function' in ticket.source ? ticket.source.test_function : undefined;
  ...
  if (ticket.relationships) {
    ...
    if (rel.children && rel.children.length > 0) parts.push(`  children: ${rel.children.join(', ')}`);
    if (rel.depends_on && rel.depends_on.length > 0) parts.push(`  depends_on: ${rel.depends_on.join(', ')}`);
    if (rel.blocks && rel.blocks.length > 0) parts.push(`  blocks: ${rel.blocks.join(', ')}`);
    if (rel.related && rel.related.length > 0) parts.push(`  related: ${rel.related.join(', ')}`);
    if (rel.duplicates) parts.push(`  duplicates: ${rel.duplicates}`);
    if (rel.linked_commits && rel.linked_commits.length > 0) parts.push(`  linked_commits: ${rel.linked_commits.join(', ')}`);
  ...
  if (ticket.resolution) {
    ...
    if (ticket.resolution.test_file) {
      const fn = ticket.resolution.test_function ? `::${ticket.resolution.test_function}` : '';
