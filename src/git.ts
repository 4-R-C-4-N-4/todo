...
export function getMergeBase(b1: string, b2: string, cwd: string = process.cwd()): string {
  return exec(['merge-base', b1, b2], cwd);
}

export function getCommitsAhead(branch: string, base: string, cwd: string = process.cwd()): number {
  const out = exec(['rev-list', '--count', `${base}..${branch}`], cwd);
  return parseInt(out, 10);
}

export function isAncestor(ancestor: string, descendant: string, cwd: string = process.cwd()): boolean {
  try {
...
export function showFileAtCommit(commit: string, path: string, cwd: string = process.cwd()): string {
  return exec(['show', `${commit}:${path}`], cwd);
}

export function getLastCommitForFile(path: string, cwd: string = process.cwd()): string {
...
export function checkoutBranch(name: string, cwd: string = process.cwd()): void {
