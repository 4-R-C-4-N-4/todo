...
import type { Ticket, State, TicketType, SourceType } from './types.js';
...
export function readTicketByPrefix(repoRoot: string, prefix: string): Ticket {
  ...
  for (const file of readdirSync(openDir(repoRoot))) {
    if (file.endsWith('.json')) allFiles.push({ dir: 'open' as const, name: file.slice(0, -5) });
  }
  for (const file of readdirSync(doneDir(repoRoot))) {
    if (file.endsWith('.json')) allFiles.push({ dir: 'done' as const, name: file.slice(0, -5) });
  }

  const matches = allFiles.filter(f => f.name.startsWith(prefix));

  if (matches.length === 0) throw new NotFoundError(prefix);
  if (matches.length > 1) throw new AmbiguousIdError(prefix, matches.map(m => m.name));
...
export function listTickets(
  ...
  return tickets.filter(t => {
    ...
    if (filters.file && !(t.files ?? []).some(f => f.path === filters.file)) return false;
    return true;
  });
}
